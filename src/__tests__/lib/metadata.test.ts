...
function stringEntry(id: string, name: string, value: string): DomainPropertyEntry {
  return {
    id,
    key: { name, category: 0 },
...
describe('toMetadataFromDomainEntries', () => {
  ...
  it('merges synthetic identifier keys from the identifiers arg', () => {
    const entries: DomainPropertyEntry[] = [stringEntry('1', 'Material', 'Steel')];
    ...
    expect(md.identifiers.VERTEX_SCENE_ITEM_ID).toBe('item-uuid');
    expect(md.identifiers.VERTEX_SCENE_ITEM_SUPPLIED_ID).toBe('supplied-1');
    expect(md.identifiers.VERTEX_PART_ID).toBe('part-1');
    expect(md.identifiers.VERTEX_PART_REVISION_ID).toBe('rev-1');
    expect(md.identifiers.VERTEX_PART_REVISION_SUPPLIED_ID).toBe('rev-supplied-1');
    expect(md.properties.Material).toBe('Steel');
  });
...
describe('toMetadataFromItem (unrestricted, server REST path)', () => {
  function sceneItem({
    ...
    return {
      ...
      relationships: partRevisionId ? { source: { data: { id: partRevisionId } } } : {},
    } as unknown as SceneItemData;
  }

  it('keeps intrinsic identifiers without unrestricted session metadata', () => {
    ...
    expect(md.identifiers).toEqual({
      VERTEX_PART_REVISION_ID: 'rev-1',
      VERTEX_SCENE_ITEM_ID: 'item-uuid',
      VERTEX_SCENE_ITEM_SUPPLIED_ID: 'supplied-1',
    });
  });
  ...
  it('alphabetizes the resulting keys', () => {
    ...
    // Identifier keys live in `identifiers`, not alongside the metadata keys.
    expect(Object.keys(md.properties)).toEqual(['Alpha', 'Mu', 'Zeta']);
    expect(md.identifiers.VERTEX_SCENE_ITEM_ID).toBe('item-uuid');
  });
});
...
describe('toMetadata (stream hit)', () => {
  ...
  it('maps stream metadataProperties (case-sensitive) and identifier keys', () => {
    ...
    // Synthetic identifier keys are surfaced separately from properties.
    expect(md?.identifiers.VERTEX_SCENE_ITEM_ID).toBe('item-uuid');
    expect(md?.identifiers.VERTEX_SCENE_ITEM_SUPPLIED_ID).toBe('supplied-1');
    expect(md?.identifiers.VERTEX_PART_ID).toBe('part-1');
    expect(md?.identifiers.VERTEX_PART_REVISION_ID).toBe('rev-1');

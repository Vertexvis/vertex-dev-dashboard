...
describe('FileCollectionTable', () => {
  ...
  it('paginates file collections using the next cursor', async () => {
    ...
    server.use(
      http.get('*/api/file-collections', ({ request }) => {
        ...
        return HttpResponse.json(
          url.searchParams.get('cursor') === 'page-2' ? nextPage : firstPageWithNextCursor
        );
      })
    );
    ...
    expect(
      requests.some((search) => {
        ...
        return params.get('cursor') === 'page-2' && params.get('pageSize') === '25';
      })
    ).toBe(true);
  });
  ...
  it('sorts by name and toggles the sort direction', async () => {
    ...
    expect(await screen.findByText('Collection Two')).toBeInTheDocument();
    expect(
      requests.some((search) => new URLSearchParams(search).get('sort') === 'name')
    ).toBe(true);
  ...
  it('clears the selection after deleting a file collection successfully', async () => {
    ...
    expect(screen.queryByText('Could not delete collection-1.')).not.toBeInTheDocument();
  });
  ...
  it('filters file collections by supplied ID before rendering the filtered results', async () => {
    ...
    server.use(
      http.get('*/api/file-collections', ({ request }) => {
        ...
        return HttpResponse.json(
          url.searchParams.get('suppliedId') === 'supplied-2' ? nextPage : firstPage
        );
      })
    );
    ...
    expect(
      requests.some((search) => {
        const params = new URLSearchParams(search);
        return (
          params.get('pageSize') === '25' && params.get('suppliedId') === 'supplied-2'
        );
      })
    ).toBe(true);
  });
  ...
  it('keeps the selection visible when deleting a file collection fails', async () => {
    ...
    expect(await screen.findByText('Could not delete collection-1.')).toBeInTheDocument();
  ...
  it('filters file collections by a partial supplied ID', async () => {
    ...
    expect(
      requests.some((search) => {
        const params = new URLSearchParams(search);
        return params.get('pageSize') === '25' && params.get('suppliedId') === 'LIED-2';
      })
    ).toBe(true);
  });
  ...
  it('renders an empty filtered result', async () => {
    server.use(
      http.get('*/api/file-collections', ({ request }) => {
        ...
        return HttpResponse.json(
          url.searchParams.get('name') === 'missing' ? emptyCollectionPage : firstPage
        );
      })
    );
    ...
    expect(await screen.findByText('No file collections found.')).toBeInTheDocument();
  });
  ...
  it('filters file collections by an inclusive created date range', async () => {
    ...
    server.use(
      http.get('*/api/file-collections', ({ request }) => {
        ...
        return HttpResponse.json(
          url.searchParams.has('createdAtStart') && url.searchParams.has('createdAtEnd')
            ? nextPage
            : firstPage
        );
      })

...
describe('PropertyKeyPolicyTable', () => {
  ...
  it('links the policy name to the detail route', async () => {
    server.use(
      http.get('*/api/property-key-policies', () => HttpResponse.json(firstPage))
    );
  ...
  it('filters policies by supplied ID', async () => {
    ...
    server.use(
      http.get('*/api/property-key-policies', ({ request }) => {
        ...
        return HttpResponse.json(
          url.searchParams.get('suppliedId') === 'supplied-2' ? filteredPage : firstPage
        );
      })
    );
    ...
    expect(
      requests.some(
        (search) => new URLSearchParams(search).get('suppliedId') === 'supplied-2'
      )
    ).toBe(true);
  });
  ...
  it('shows an error, clears the selection, and refreshes the list when deleting fails', async () => {
    ...
    expect(await screen.findByText('Could not delete policy-1.')).toBeInTheDocument();
  ...
  it('reconciles a partial delete and keeps only failed policies selected', async () => {
    ...
    renderWithSWR(<PropertyKeyPolicyTable onPoliciesDeleted={onPoliciesDeleted} />);
    ...
    expect(await screen.findByText('Could not delete policy-2.')).toBeInTheDocument();
  ...
  it('recovers from a non-JSON delete error without getting stuck', async () => {
    ...
    expect(
      await screen.findByText('Could not delete the selected property key policies.')
    ).toBeInTheDocument();
    await waitFor(() => expect(getCount).toBeGreaterThan(getsBeforeDelete));
  });
  ...
  it('notifies the parent with the deleted ids on success', async () => {
    ...
    server.use(
      http.get('*/api/property-key-policies', () => HttpResponse.json(firstPage)),
      http.delete('*/api/property-key-policies', () => HttpResponse.json({ status: 200 }))
    );
    ...
    await waitFor(() => expect(onPoliciesDeleted).toHaveBeenCalledWith(['policy-1']));
  });
  ...
  it('clears the selection when the user navigates to the next page', async () => {
    ...
    server.use(
      http.get('*/api/property-key-policies', () => HttpResponse.json(firstPageWithNext))
    );

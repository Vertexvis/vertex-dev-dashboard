...
describe('property key policies API routes', () => {
  it('passes supplied ID and paging filters to Vertex', async () => {
    nodeMswServer.use(
      stubListPolicies(policyList([policyData('policy-1')]), ({ searchParams }) => {
  ...
  it('creates a policy then upserts its entries verbatim', async () => {
    ...
    expect(entriesBody).toEqual({
      data: [{ name: 'MixedCaseKey' }, { name: 'another_key' }, { name: 'UPPER' }],
    });
  });
...
function callPolicyById(id: string, req: ApiRouteRequest): Promise<ApiRouteResponse> {
...
function stubCreatePolicy(
  ...
  return http.post(`${vertexApiOrigin}/property-key-policies`, async ({ request }) => {
